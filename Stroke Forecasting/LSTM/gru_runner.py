...
from utils import evaluation_rnn, save
...
def GRU_trainer(k_fold_index, data_loader, val_data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config, device="cpu"):
    ...
    for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
        ...
        for loader_idx, item in enumerate(data_loader):
            ...
            hidden = torch.zeros(config['n_layers'] * config['num_directions'], batch_input_shot.shape[0], config['encode_dim']).to(device)
            cell = torch.zeros(config['n_layers'] * config['num_directions'], batch_input_shot.shape[0], config['encode_dim']).to(device)
            ...
            encoder_output, hidden, cell = encoder(input_shot, input_x, input_y, input_player, hidden, cell)
            ...
            output_xy, output_shot_logits, hidden, cell = decoder(input_shot, input_x, input_y, input_player, target_player, hidden, cell)
        ...
        # # validation
        # val_loss = evaluation_rnn(val_data_loader, encoder, decoder, criterion, config, config['model_type'], samples=1, device=device)
...
def GRU_generator(given_seq, correct_seq, encoder, decoder, criterion, config, samples, device):
    encode_length = config['encode_length']
    ...
    generated_shot, generated_area = [], []
    prev_shot, prev_area, prev_player = None, None, None
    ...
    with torch.no_grad():
        ...
        hidden = torch.zeros(config['n_layers'] * config['num_directions'], input_shot.shape[0], config['encode_dim']).to(device)
        cell = torch.zeros(config['n_layers'] * config['num_directions'], input_shot.shape[0], config['encode_dim']).to(device)
    ...
    return loss, total_instance, generated_area, generated_shot
