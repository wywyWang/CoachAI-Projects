...
def shotGen_trainer(k_fold_index, data_loader, val_data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config, device="cpu"):
...
def shotgen_generator(given_seq, correct_seq, encoder, decoder, criterion, config, samples, device):
    encode_length = config['encode_length']
    ...
    with torch.no_grad():
        ...
        for sample_id in range(samples):
            ...
            for seq_idx in range(encode_length, len(given_seq['given_shot'])):
                ...
                output_xy, output_shot_logits, decoder_self_attention_list, decoder_encoder_self_attention_list, disentangled_weight_local = decoder(input_shot, input_x, input_y, input_player, encode_local_output, encode_global_A, encode_global_B, target_player, return_attns=True)
            ...
            if current_loss['total'] <= loss['total']:
                ...
                generated_decoder_attention = decoder_self_attention_list.cpu().detach().numpy()
                generated_decoder_encoder_attention = decoder_encoder_self_attention_list.cpu().detach().numpy()
                generated_decoder_encoder_disentangled = disentangled_weight_local.copy()

                generated_shot, generated_area = current_generated_shot.copy(), current_generated_area.copy()

    return loss, total_instance, generated_area, generated_shot, generated_decoder_attention, generated_decoder_encoder_attention, generated_decoder_encoder_disentangled
