...
def initialize_adjacency_matrix(batch_size, encode_length, shot_type):
    adjacency_matrix = torch.zeros((14, encode_length * 2, encode_length * 2), dtype=int).to(shot_type.device)
    ...
    for row in range(encode_length * 2):
        ...
        if row % 2 == 0: # black node
            ...
            if node_index % 2 == 1: # odd black node
                if (node_index + 1) * 2 <= encode_length * 2 - 1:
                    adjacency_matrix[13][row][(node_index + 1) * 2] = 1
                    adjacency_matrix[13][(node_index + 1) * 2][row] = 1
        if row % 2 == 1: # white node
            if node_index % 2 == 0: # even white node
                if (node_index + 1) * 2 + 1 <= encode_length * 2 - 1:
                    adjacency_matrix[13][row][(node_index + 1) * 2 + 1] = 1
                    adjacency_matrix[13][(node_index + 1) * 2 + 1][row] = 1
...
def update_adjacency_matrix(batch_size, step, adjacency_matrix, shot_type_predict=False):
    new_adjacency_matrix = torch.zeros((batch_size, 14, step * 2, step * 2), dtype=int).to(adjacency_matrix.device)
...
class Decoder(nn.Module):
    def __init__(self, args, device):
        ...
        self.predict_xy = nn.Linear(hidden_size*2, 2 * 5 * args['num_GMM']) # 2 players * 5(x, y, sx, sy, corr) * gmm_num
    ...
    def forward(self, player, step, encode_node_embedding, original_embedding, adjacency_matrix, 
        ...
        predict_xy = predict_xy.view(batch_size, 2, 5 * self.num_GMM)
