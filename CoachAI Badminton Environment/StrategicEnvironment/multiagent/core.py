...
class Agent(Entity):
    def __init__(self, index, time_scale):
        ...
        try:
            self.palyer_speed_model = pd.read_csv(r'StrategicEnvironment\Data\player_speed_model.csv',index_col=0)
        except:
            self.palyer_speed_model = pd.read_csv(r'rl-environment\Data\player_speed_model.csv',index_col=0)
    ...
    def generate_speed(self, target_pos):
        ...
        if self.agent_index == 0:
            if self.target_pos[1] > 480:
                self.target_pos[1] = 480-10
            if self.target_pos[1] < 150-5:
                self.target_pos[1] = 150
        else:
            if self.target_pos[1] < 480:
                self.target_pos[1] = 480+10
            elif self.target_pos[1] > 810+5:
                self.target_pos[1] = 810

        if self.target_pos[0] < 50:
            self.target_pos[0] = 50
        if self.target_pos[0] > 305:
            self.target_pos[0] = 305     
...
class World(object):
    ...
    def step(self,action_n):
        ...
        ball.ball_type = launcher_action['ball_type']
        ball.last_ball = launcher_action['last_ball']
    ...
    def check_step_done(self, ball, catcher):
        ...
        # Check 
        # if ball.state.p_pos[1] < 150 or ball.state.p_pos[1] > 810 or ball.state.p_pos[0] < 50 or ball.state.p_pos[0] > 305:
        #     return True, 
...
class Ball(Entity):
    def __init__(self, n, g, time_scale):
        ...
        try:
            ...
            self.ball_initial_state_model = pd.read_csv(r'StrategicEnvironment\Data\initial_state\ball_initial_state_model.csv',index_col=0)
            self.ball_type_frame_distribution = pd.read_csv(r'StrategicEnvironment\Data\ball_type_model.csv',index_col=0)
            self.hit_height = pd.read_csv(r'StrategicEnvironment\Data\hit_height.csv',index_col='type')
            self.hit_range = pd.read_csv(r'StrategicEnvironment\Data\hit_range.csv',index_col='type')
    ...
    def returnable(self, distance):
        ...
        if distance>self.returnable_distance:
            ...
            return False
        # Will fall outside
        if self.target_pos[0] > 310 or self.target_pos[0] < 55 or self.target_pos[1] < 155 or self.target_pos[1] > 815 :
            #print('outside')
            return False
