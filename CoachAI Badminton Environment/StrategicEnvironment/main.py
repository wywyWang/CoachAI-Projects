...
def main():
    ...
    env = MultiAgentEnv(world, scenario.reset_world, scenario.reward, scenario.observation, info_callback=None, top_viewer = True, side_viewer = False)

    # set policy for each agent
    env.render()

    policies = []
    policies.append(Player_Policy(env,0, filePath,'A'))
    policies.append(Player_Policy(env,1, filePath,'B'))
